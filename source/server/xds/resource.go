...
// buildHTTPRule 构建 HTTP 协议的 EDS、CDS、RDS 和 LDS 四类 Envoy 资源。
func buildHTTPRule(rule *ProxyRule, connectTimeout time.Duration) (*ruleRes, error) {
	...
	return &ruleRes{
		endpoint: ep, cluster: cl, route: rt, listener: li,
	}, nil
}

// ─── UDP ───────────────────────────────────────────────────────────────

// buildUDPRule 构建 UDP 协议的 CDS 和 LDS 资源，使用 UDP 代理过滤器。
func buildUDPRule(rule *ProxyRule, connectTimeout, udpIdleTimeout time.Duration) (*ruleRes, error) {
	...
	return &ruleRes{
		endpoint: nil, cluster: cl, route: nil, listener: li,
	}, nil
}
